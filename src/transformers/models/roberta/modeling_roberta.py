...
class RobertaOutput(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, input_tensor: torch.Tensor) -> torch.Tensor:
        ...
        hidden_states = self.LayerNorm(hidden_states + input_tensor)
        return hidden_states
class RobertaOutput2(nn.Module):
    def __init__(self,
                 output: RobertaOutput, config):
        super(RobertaOutput2, self).__init__()
        self.output = output
    ...
    def forward(self, hidden_states: torch.Tensor, input_tensor: torch.Tensor):
        ...
        hidden_states = self.output.LayerNorm(hidden_states + input_tensor)
        return hidden_states
...
def adaptoutput(config):
    return lambda output: RobertaOutput2(output,config=config)
def RobertaModel2(model,config):
    for layer in model.roberta.encoder.layer:
        layer.output = adaptoutput(config)(layer.output)
    for param in model.parameters():
        param.requires_grad = False
    for name, sub_module in model.roberta.named_modules():
