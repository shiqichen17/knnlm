...
from ...modeling_outputs import (
    ...
    CausalLMOutputWithCrossAttentions2,
...
class GPT2Block(nn.Module):
    def __init__(self, config,totalnumber,layer_idx=None):
        super().__init__()
...
class GPT2Model(GPT2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.embed_dim = config.hidden_size
        totalnumber=config.num_hidden_layers

        self.wte = nn.Embedding(config.vocab_size, self.embed_dim)
        self.wpe = nn.Embedding(config.max_position_embeddings, self.embed_dim)

        self.drop = nn.Dropout(config.embd_pdrop)
        self.h = nn.ModuleList([GPT2Block(config, totalnumber,layer_idx=i) for i in range(config.num_hidden_layers)])
...
class GPT2LMHeadModel2(GPT2PreTrainedModel):
    _keys_to_ignore_on_load_missing = [r"attn.masked_bias", r"attn.bias", r"lm_head.weight"]
